...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT * FROM calon_mahasiswa WHERE email='$email' LIMIT 1";
    $result = mysqli_query($conn, $query);

    if (mysqli_num_rows($result) == 1) {
        $user = mysqli_fetch_assoc($result);
        $stored = $user['password'] ?? '';
        $is_valid = password_verify($password, $stored) || $stored === $password;
...
<head>
    <meta charset="UTF-8">
    <title>Login Calon Mahasiswa - PMB</title>
...
<body>
    ...
    <div class="page">
        <h1 class="login-title">Login Akun PMB</h1>
        ...
        <div class="login-wrapper">
            ...
            <div class="register-link">
                Belum punya akun?
                <a href="register.php">Daftar Calon Mahasiswa</a>
            </div>
        </div>
    </div>
